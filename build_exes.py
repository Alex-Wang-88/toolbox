...
SPEC = os.path.join(ROOT, "config", "TOOLBOX.spec")
...
def main() -> None:
    results = []
    results.append(("gpu", "TOOLBOX", build_variant("gpu", "TOOLBOX")))
    results.append(("cpu", "TOOLBOX_CPU", build_variant("cpu", "TOOLBOX_CPU")))
    ...
    if not ok:
        ...
        sys.exit(1)
    print("\n[OK] 两个变体均已构建完成：dist/TOOLBOX.exe 与 dist/TOOLBOX_CPU.exe")
